...
export function ConferenceBacking() {
  ...
  return (
    <article className="relative border-t border-line pt-12 md:pt-16">
      <div className="grid gap-10 lg:grid-cols-[0.88fr_1.12fr] lg:gap-14 lg:items-start">
        {/* Brand column */}
        <div className="relative overflow-hidden bg-bg-elevated/50 px-6 py-10 sm:px-8 sm:py-12">
          <div
            className="pointer-events-none absolute -right-16 -top-20 h-48 w-48 rounded-full bg-accent/10 blur-3xl"
            aria-hidden
          />
          <div
            className="pointer-events-none absolute -bottom-20 -left-12 h-52 w-52 rounded-full bg-signal/8 blur-3xl"
            aria-hidden
          />
          <div className="relative">
            ...
            <p className="mt-6 text-xs font-semibold tracking-[0.14em] text-signal uppercase">
              {site.coverage}
            </p>
          </div>
        </div>
        ...
        <div className="min-w-0 lg:pt-1">
          ...
          <ul className="mt-10 divide-y divide-line border-y border-line">
            {backing.capabilities.map((item, index) => (
              <li
                key={item.label}
                className="group grid gap-1 py-5 transition sm:grid-cols-[9.5rem_1fr] sm:gap-6 sm:items-baseline"
              >
                ...
                <p className="text-sm leading-relaxed text-muted pl-7 sm:pl-0">
                  {item.detail}
                </p>
              </li>
            ))}
